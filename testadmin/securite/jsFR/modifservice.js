...
function changetextPC(site) {
  ...
  for (let k = 0; k < site["Francais"]["numbimg"]; k++) {
    imgedit.innerHTML = imgedit.innerHTML+'<img class="photoService" id="photoServiceEdit' + k + '" src="Service' + document.getElementById("number").value + ':' + k + '" >';
  }
  let imgNumb = document.getElementById('Numbimg')
  let img = document.getElementById('editPhoto')
  let k = 0;
  if (site["Francais"]["numbimg"] != 0) {
    for (k = 0; k < site["Francais"]["numbimg"]; k++) {
      img.innerHTML = img.innerHTML + '<input type="file" id="photoService' + k + '" name="photoService' + k + '" >';
    }
    imgNumb.value = k;
  } else {
    img.innerHTML = img.innerHTML + '<input type="file" id="photoService' + k + '" name="photoService' + k + '" >';
    ...
    document.getElementById('imgB+').addEventListener('click', function () {
      ...
      imgnew.type = 'file';
      imgnew.id = "phoroService" + k;
      imgnew.name = "photoService" + k;
      img.append(imgnew);
      k++;
      imgNumb.innerHTML = k;
    })
