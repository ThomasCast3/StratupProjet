...
function changetextP(site){
    ...
    document.getElementById('text1').innerHTML = deleteslash(site[langage]["text1"]);
    document.getElementById('text2').innerHTML = deleteslash(site[langage]["text2"]);
}
function changetextPC(site){
    let Photo = document.getElementById('containerPhoto')
    Photo.innerHTML="";
    for (let i = 0; i < site["Francais"]["numbimg"]; i++) {
        Photo.innerHTML = Photo.innerHTML + '<img id="photoService' + i + '" class="photoService" src="/img/Serv4-'+i+'.">';
    }
}
...
function format(site){
    for (k = 0; k < document.getElementsByClassName("photoService").length; k++) {
        document.getElementById("photoService" + k).src = document.getElementById("photoService" + k).src + site['Francais']['Serv'+k];
    }
}
...
if (langage === "Francais") {
    ...
    fetch(requestURL)
        ...
        .then(jsonData => {
            ...
            requestURL = "/fr/jsonFR/service4cross.json";
            fetch(requestURL)
                .then(response => response.json())
                .then(jsonData => {
                    ...
                    fetch(requestURL)
                        ...
                        .then(jsonData => {
                            formatG(jsonData)
                            requestURL = "/fr/jsonFR/formatService4.json";
                            fetch(requestURL)
                                .then(response => response.json())
                                .then(jsonData => {
                                    format(jsonData)
                                    ...
                                    fetch(requestURL)
                                        ...
                                        .then(jsonData => {
                                            changetext(jsonData)
                                            requestURL = "/fr/jsonFR/service4.json";
                                            fetch(requestURL)
                                                .then(response => response.json())
                                                .then(jsonData => {
                            ...
                            fetch(requestURL)
                                ...
                                .then(jsonData => {
                                    ...
                                    fetch(requestURL)
                                        .then(response => response.json())
                                        .then(jsonData => {
                                            document.getElementById("NomServ").innerHTML = "Feasibility studies, pre-projects and detailed studies";
                                            ...
                                            changetext(jsonData)
                                            requestURL = "/en/jsonEN/service4.json";
                                            fetch(requestURL)
                                                .then(response => response.json())
                                                .then(jsonData => {
                                                    changetextP(jsonData)
