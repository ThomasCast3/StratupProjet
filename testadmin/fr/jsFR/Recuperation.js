...
function changetextPC(site){
    document.getElementById('imgProduit').innerHTML = `<img class="imgBrochur" src="/img/Recuperation.png">`;
...
requestURL = "/fr/jsonFR/Recuperationcross.json";
fetch(requestURL)
    .then(response => response.json())
    .then(jsonData => {
...
if (langage === "Francais") {
    ...
    fetch(requestURL)
        .then(response => response.json())
        .then(jsonData => {
            document.getElementById("NomProd").innerHTML = "Energy recovery";
